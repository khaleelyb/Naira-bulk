  ...
  <div className="space-y-3">
    ...
    <div className="flex gap-2">
      ...
      <select value={orderStatusFilter} onChange={e => setOrderStatusFilter(e.target.value)}
        className="px-2 py-2.5 text-sm bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl text-gray-700 dark:text-gray-300 focus:outline-none focus:ring-2 focus:ring-green-400 min-w-0 w-28">
    ...
    {orderStatusFilter === 'all' && pendingShipment.length > 0 && (
      <div className="flex items-center gap-3 bg-green-50 dark:bg-green-900/10 border border-green-200 dark:border-green-800/40 rounded-xl px-4 py-3 cursor-pointer"
        ...
        <p className="text-sm text-green-700 dark:text-green-400 flex-1">
          <strong>{pendingShipment.length}</strong> paid order{pendingShipment.length !== 1 ? 's' : ''} need shipping
        </p>
        <span className="text-xs font-bold text-green-600 whitespace-nowrap">Filter →</span>
      </div>
    )}
    ...
    <div className="flex items-center justify-between px-1">
      ...
      <span className="text-xs text-gray-400">₦{filteredOrders.reduce((s, o) => s + (o.status !== 'failed' ? o.amount : 0), 0).toLocaleString()}</span>
    </div>
        ...
        return (
          <div key={o.id} className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 overflow-hidden">
            ...
            <div className="flex items-start gap-3 p-3">
              ...
              <p className="font-bold text-green-600 dark:text-green-400 text-base flex-shrink-0">
            ...
            <div className="grid grid-cols-2 gap-x-3 gap-y-1.5 px-3 py-2.5 bg-gray-50 dark:bg-gray-800/50 border-t border-gray-100 dark:border-gray-800">
              {(o.buyerName || buyer?.name) && (
                <div className="flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400 col-span-2">
                  ...
                  <span className="font-medium text-gray-700 dark:text-gray-300 truncate">{o.buyerName ?? buyer?.name}</span>
                </div>
              )}
              {o.buyerPhone && (
                <a href={`tel:${o.buyerPhone}`} className="flex items-center gap-1.5 text-xs text-green-600 dark:text-green-400 hover:underline">
            ...
            <div className="flex items-center gap-2 px-3 py-2.5 border-t border-gray-100 dark:border-gray-800">
              {nextStatus && (
                <button onClick={() => onUpdateOrderStatus(o.id, nextStatus)}
                  className="flex-1 text-xs font-bold bg-green-500 hover:bg-green-600 text-white px-3 py-2 rounded-xl transition-colors">
                  {NEXT_LABEL[o.status]}
                </button>
              )}
              ...
              {seller?.phone && (
                <button
                  onClick={() => {
                    ...
                    const text = [`Hello ${seller.name},`, ``, `📦 New Order`, `Product: ${o.productTitle}`, `Amount: ₦${o.amount.toLocaleString()}`, ``, `👤 Buyer: ${o.buyerName ?? '—'}`, o.buyerPhone ? `📞 ${o.buyerPhone}` : '', o.buyerAddress ? `📍 ${o.buyerAddress}` : ''].filter(Boolean).join('\n');
                  ...
                  {seller && (
                    <div className="bg-gray-50 dark:bg-gray-800/60 rounded-xl p-2.5 col-span-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-gray-600 dark:text-gray-300">{seller.name} (@{seller.username})</span>
                      </div>
                    </div>
                  )}
                  <div className="bg-gray-50 dark:bg-gray-800/60 rounded-xl p-2.5">
                    <p className="text-gray-400 font-semibold uppercase tracking-wide mb-1">Amount</p>
                    <p className="font-bold text-gray-600 dark:text-gray-300">₦{o.amount.toLocaleString()} {o.currency}</p>
                  </div>
                  <div className="bg-gray-50 dark:bg-gray-800/60 rounded-xl p-2.5">
                    <p className="text-gray-400 font-semibold uppercase tracking-wide mb-1">Updated</p>
                    <p className="text-gray-600 dark:text-gray-300">{formatDate(o.updatedAt)}</p>
                  </div>
                </div>
                <div className="bg-gray-50 dark:bg-gray-800/60 rounded-xl p-2.5">
                  <p className="text-xs text-gray-400 font-semibold uppercase tracking-wide mb-2">Update status</p>

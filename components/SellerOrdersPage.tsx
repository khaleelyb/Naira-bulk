          ...
          {state === 'ok' ? (
            <div className="flex flex-col items-center py-4 gap-3">
              ...
              <div className="text-center">
                ...
                <p className="text-sm text-gray-400 mt-1">Order marked as delivered.</p>
              </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800/40 rounded-xl p-3 text-xs text-amber-700 dark:text-amber-400">
                Ask the buyer for their 6-digit delivery code. Enter it below to confirm delivery and release the order.
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full text-center text-2xl font-bold tracking-[0.4em] py-3 px-4 rounded-xl border transition-all focus:outline-none focus:ring-2 ${
                    state === 'wrong' || state === 'error'
                      ? 'border-red-300 dark:border-red-700 bg-red-50 dark:bg-red-900/10 text-red-600 focus:ring-red-400'
                      : 'border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-green-400 focus:border-green-400'
                  }`}
                  autoFocus
                />
                {state === 'wrong' && (
                  <p className="text-xs text-red-500 mt-1.5 flex items-center gap-1">
                    ...
                    Incorrect code. Ask the buyer to check their order history.
                  </p>
                )}
                {state === 'error' && (
...
export const SellerOrdersPage: React.FC<SellerOrdersPageProps> = ({
  ...
  const [localOtpResults, setLocalOtpResults] = useState<Record<string, 'ok' | 'wrong' | 'error'>>({});

  // Only show this seller's orders
  ...
  const handleOtpConfirm = async (order: Order, enteredOtp: string): Promise<'ok' | 'wrong' | 'error'> => {
    // Compare against the delivery_otp stored on the order
    if (!order.deliveryOtp) return 'error';
    if (order.deliveryOtp !== enteredOtp) return 'wrong';
    // Mark as delivered
    onUpdateOrderStatus(order.id, 'delivered');
    setLocalOtpResults(prev => ({ ...prev, [order.id]: 'ok' }));
    return 'ok';
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-6 space-y-5">
        ...
        {/* ── OTP delivery banner ── */}
        {stats.paid > 0 && (
          <div className="flex items-center gap-3 bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800/40 rounded-2xl px-4 py-3">
            ...
            <p className="text-sm text-amber-700 dark:text-amber-400 font-medium flex-1">
              <strong>{stats.paid}</strong> paid order{stats.paid !== 1 ? 's' : ''} ready to ship. When you deliver, tap <strong>"Confirm Delivery"</strong> and enter the buyer's code.
            </p>
          </div>
        )}
        ...
        {filtered.length === 0 ? (
          ...
          <div className="space-y-3">
            {filtered.map(order => {
              ...
              const canConfirmDelivery = (order.status === 'shipped' || order.status === 'success') && order.deliveryOtp && !order.otpVerified;

...
import { Upload, Link as LinkIcon, Send, Globe, AlertCircle, Loader2 } from 'lucide-react';
            ...
            <div className="p-8 sm:p-10 space-y-8">
              ...
              <div className="space-y-4">
                ...
                <div className="relative">
                  <LinkIcon className="absolute left-4 top-3.5 h-4 w-4 text-slate-300" />
                  <input
                    {...register('productUrl')}
                    className="w-full bg-slate-50 border border-slate-100 rounded-2xl py-3.5 pl-12 pr-4 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-[#FF5A00]/20 transition-all"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-4">
                  ...
                  <div className="relative group cursor-pointer border-2 border-dashed border-slate-200 rounded-[24px] hover:border-[#FF5A00] hover:bg-[#FF5A00]/5 transition-all aspect-video flex flex-col items-center justify-center p-4">
                    ...
                    {imagePreview ? (
                      <img src={imagePreview} className="w-full h-full object-contain rounded-lg shadow-sm" alt="Preview" />
                    ) : (
                      <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 pt-6 border-t border-slate-100">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 border border-slate-100 rounded-xl py-3 px-4 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-[#FF5A00]/20"
                  />
                  {errors.quantity && <p className="text-[10px] font-bold text-red-500">{errors.quantity.message}</p>}
                </div>
                <div className="space-y-2">
                  <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest">Courier</label>
                  <select
                    {...register('shippingMethod')}
                    className="w-full bg-slate-50 border border-slate-100 rounded-xl py-3 px-4 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-[#FF5A00]/20 appearance-none"
                  >
                    <option value="Air Freight (Express)">Air (Express)</option>
                    <option value="Air Freight (Standard)">Air (Standard)</option>
            ...
            <div className="space-y-10">
              {[
                ...
                <div key={idx} className="flex gap-4 group">
                  <div className="h-10 w-10 flex items-center justify-center rounded-xl bg-slate-50 text-slate-400 font-bold text-xs group-hover:bg-[#FF5A00]/10 group-hover:text-[#FF5A00] transition-colors shrink-0">
                    {step.num}
                  </div>
                  <div>

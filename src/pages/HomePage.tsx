...
import { ArrowRight, Zap, ShieldCheck, Truck, BarChart3, Globe } from 'lucide-react';
    ...
    <div className="space-y-24 pb-32">
          ...
          >
            ...
            <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-3 max-w-2xl mx-auto mb-10">
              <input
                ...
                className="flex-1 h-14 bg-white border border-slate-200 rounded-full px-6 text-sm font-medium shadow-soft focus:outline-none focus:ring-2 focus:ring-[#FF5A00]/20"
              />
              <button
                type="submit"
                className="h-14 px-8 bg-slate-900 text-white font-bold rounded-full hover:bg-black transition-all whitespace-nowrap"
              >
                Search
              </button>
            </form>
            ...
            <div className="flex flex-col sm:flex-row gap-5 justify-center">
              <Link
                to="/search"
                className="px-12 py-5 bg-slate-900 text-white font-bold rounded-full hover:bg-black transition-all flex items-center justify-center gap-4 group shadow-2xl shadow-slate-900/10 active:scale-95"
              >
                Explore Marketplace
              ...
              <Link
                to="/import"
                className="px-12 py-5 bg-white text-slate-900 font-bold rounded-full border border-slate-200 hover:border-slate-900 transition-all flex items-center justify-center gap-4 active:scale-95"
              >
                Custom Procurement
      ...
      <section className="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-4 gap-8">
        {TRUST_BADGES.map((item) => (
          <div key={item.title} className="group p-2">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-6">
          {CATEGORIES.map((cat) => (
            <button
              ...
              className="flex flex-col items-center gap-5 p-8 bg-white rounded-[40px] border border-slate-100 hover:border-[#FF5A00] hover:shadow-soft transition-all group"
            >
      ...
      <section className="max-w-7xl mx-auto px-4">
        <div className="flex items-center justify-between mb-12">
          <div className="space-y-1">
            ...
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Selected from top global vendors</p>
          </div>
          <Link
            to="/search"
            className="text-[10px] font-bold text-[#FF5A00] uppercase tracking-widest border border-slate-100 px-6 py-2.5 rounded-full hover:bg-slate-50 transition-all"
          >
            View All Products
          </Link>
        </div>
        ...
        {loading ? (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {Array.from({ length: 4 }).map((_, n) => (
              <div key={n} className="aspect-[3/4] bg-slate-50 border border-slate-100 animate-pulse rounded-[32px]" />
            ))}
          </div>
        ) : products.length === 0 ? (
          <div className="text-center py-20 text-slate-400 font-medium">No products yet. Check back soon.</div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
      ...
      <section className="max-w-7xl mx-auto px-4">
        <div className="bg-slate-900 rounded-[64px] p-12 sm:p-24 text-white relative overflow-hidden group shadow-2xl shadow-slate-900/10">
          ...
          <div className="relative z-10 max-w-2xl">
            ...
            <h2 className="text-5xl sm:text-6xl font-bold mb-8 tracking-tighter leading-[0.95]">
              Have a link from <br /> Alibaba or 1688?
            </h2>
            ...
            <Link
              to="/import"
              className="inline-flex items-center gap-3 bg-[#FF5A00] text-white px-10 py-5 rounded-full font-bold text-xs uppercase tracking-[0.2em] hover:bg-[#E65100] transition-all shadow-xl shadow-[#FF5A00]/20"
            >
